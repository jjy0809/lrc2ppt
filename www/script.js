...
function showStep(step) {
    ...
    updateIframe(step);
    scrollToTop();
    document.getElementById('downPercent').style.display = 'none';
...
function updateProgressText(text) {
    const element = document.getElementById('downPercent');
...
async function generatePresentation(options) {
    ...
    // 1. 레이아웃 설정 시스템
    const LAYOUT_CONFIG = {
        'wide': { name: 'LAYOUT_WIDE', size: [13.3, 7.5] },
        '16x9': { name: 'LAYOUT_16x9', size: [10, 5.625] },
        '16x10': { name: 'LAYOUT_16x10', size: [10, 6.25] },
        '4x3': { name: 'LAYOUT_4x3', size: [10, 7.5] }
    };
    const layout = LAYOUT_CONFIG[options.layout] || LAYOUT_CONFIG['16x9'];
    pres.layout = layout.name;
    const [SLIDE_W, SLIDE_H] = layout.size; // 실제 슬라이드 크기(인치)
    ...
    updateProgressText('가사를 옵션에 맞게 처리하는 중');
    ...
    try {
        ...
        updateProgressText(`${processedLyrics.length}장의 슬라이드를 생성하는 중`);

        // 3. 모든 슬라이드 생성 로직
        processedLyrics.forEach(text => {
            const slide = pres.addSlide({ masterName: 'MASTER_SLIDE' });

            // 3-1. 메인 텍스트 박스 (절대적 중앙 배치)
            slide.addText(text, {
                x: (SLIDE_W * 0.05),  // 좌측 마진 5%
                y: (SLIDE_H * 0.05),  // 상단 마진 5%
                w: SLIDE_W * 0.9,     // 가로 90%
                h: SLIDE_H * 0.9,     // 세로 90%
                ...
                lineSpacingMultiple: options.lineSpace,
                color: rgbToHex(options.fontColor),
                fontFace: options.fontFace,
                autoFit: { 
                    shrinkText: true,
                    maxLines: options.lineMode === 'double' ? 2 : (options.autoWrap ? 2 : 1)
                }
            });
            ...
            if (options.title.enable && options.title.text) {
                ...
                switch (options.title.hPosition) { 
                    case 'left':
                        ...
                        break;
                    case 'right':
                        xPos = SLIDE_W - TITLE_WIDTH - MARGIN;
                        break;
                    case 'center':
                        xPos = (SLIDE_W - TITLE_WIDTH) / 2;
                        break;
                }
            
                let yPos;
                switch (options.title.vPosition) {
                    case 'top':
                        yPos = MARGIN;
                        break;
                    case 'bottom':
                        yPos = SLIDE_H - TITLE_HEIGHT - MARGIN;
                        break;
                }
                ...
                slide.addText(options.title.text, {
                    ...
                    align: { 
                        ...
                        center: 'center', 
                        right: 'right' 
                    }[options.title.hPosition] || 'center',
                    fontFace: options.fontFace
                });
            }
        });
...
async function generateFinalPPT() {
    ...
    document.getElementById('downPercent').style.display = 'block';
    updateProgressText('옵션 분석 중');
    ...
    try {
        appData.options = {
            ...
            fontSize: parseInt(document.getElementById('fontSize').value) || 45,
            lineSpace: parseFloat(document.getElementById('spaceSize').value) || 1.35,
            fontColor: hexToRgb(document.getElementById('fontColor').value) || [255,255,255],
            layout: document.getElementById('slideLayout').value || '16x9',
            bgColor: hexToRgb(document.getElementById('bgColor').value) || [0,0,0],
            ...
            horizontalAlign: document.getElementById('lyricsHPosition').value || 'center',
            verticalAlign: document.getElementById('lyricsVPosition').value || 'middle',
            title: {
        ...
        updateProgressText('PPT 생성 완료');
        link.click();
    } catch (error) {
        alert(`❗ PPT 생성 오류 - ${error.message}`);
        updateProgressText('PPT 생성 실패');
        console.error(error);
    } finally {
        generateBtn.disabled = true;
...
function loadOptionsFromStorage() {
    ...
    document.getElementById('slideLayout').value = options.layout;
    ...
    document.getElementById('lyricsVPosition').value = options.verticalAlign;
    document.getElementById('lyricsHPosition').value = options.horizontalAlign;
    document.getElementById('spaceSize').value = options.lineSpace;
    
    if(options.bgImage) {
        const blob = dataURItoBlob(options.bgImage);
        const file = new File([blob], "이전에 사용한 배경 이미지.jpg", {type: "image/jpeg"});
...
async function saveOptions() {
    appData.options = {
        ...
        title: {
            ...
            vPosition: document.getElementById('titleVPosition').value || 'top',
            fontSize: parseInt(document.getElementById('titleFontSize').value) || 30,
            color: hexToRgb(document.getElementById('titleColor').value) || [255,255,255]
        }
    };
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'F9' && appData.currentStep === 3) {
        document.getElementById('finalGenerateBtn').disabled = false;
        generateFinalPPT();
    }
});
