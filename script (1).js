...
async function fetchYouTubeCaption() {
    const urlInput = document.getElementById('youtubeURL');
    const button = document.querySelector('#youtubeURLOption .icon-button');
    ...
    try {
        ...
        const API_KEY = 'AIzaSyBL_########6VcLX6xdDmnrLo9I';
        const CLIENT_ID = '6856866146###########2nuppn4s0p9mgnfh7.apps.googleusercontent.com';
        ...
        if (!response.ok) throw new Error('자막을 찾을 수 없습니다');
        const data = await response.json();
        appData.lyrics = data.items[0].snippet.text.split('\n');
        showStep(2);
    } catch (error) {
        alert(`❗${error}`);
    } finally {
